...
For formal work after stable identity and package decomposition, derive packages from the accepted contract and shared package envelope. The runtime Journal owns Agent/job/turn/join/settlement state. Optional `formal-task-board.md` notes and free-form `progress.txt` are continuity aids only; missing or arbitrary content never blocks and is never parsed as authority. Phase readiness consumes accepted dependencies, exact owner, runtime settlement, portable evidence, ROSE inspection, disposition, and verification links. A worker return, runtime status, progress note, or checked box cannot independently satisfy a phase gate.
...
Ordinary one-turn/report-only work does not invoke durable memory. Required MemPalace use needs compatible provider detection, supported configured adapter capability, canonical user Palace/Wing/diary mapping, a separate exact operation approval, and no possible concurrent supported writer; otherwise it fails closed without a fallback. Concurrent-write safety remains `Unverified`. Handoff likewise requires explicit user intent or an already accepted lifecycle trigger, uses a repository-local redacted reference-first artifact, and promotes no durable memory by default. For A33, handoff/checkpoint records owning-repository destinations and preserved rollback worktree/evidence references; every later ADD or non-force REMOVE requires a new exact operation/key/class-bound approval and no continuity artifact broadens access.

## Optional Capability Gates

Run the proactive delegation scan across commands and natural-language routes. Prefer dispatch for a clear bounded non-trivial package with a matching available specialist and current effective permissions/capabilities; direct work remains only for trivial work, contract clarification or splitting, no matching specialist, permission/capability failure, overlapping ownership, or concrete negative benefit. Do not create a parallelism report, ownership table, research chain, or stress-test merely because work has multiple steps, files, artifacts, or packages.

- Dispatch concurrent tools or Task only after a clear bounded non-trivial package has a matching available specialist and current effective permissions/capabilities. One auxiliary capability defaults to at most two contexts, but this is not a hard cap; ROSE chooses any larger bounded fan-out only for independent non-overlapping units with concrete benefit, suitable owners, and an explicit join plan. Otherwise apply one of the named direct-work exceptions without user-facing ceremony.
...
- Actions: run the proactive delegation scan, explore alternatives, identify assumptions, and collect evidence needs. Under `direct-vs-delegated-work.md`, a clear bounded non-trivial package with a matching available specialist and current permissions/capabilities dispatches promptly; work directly only for a named direct-work exception. When a candidate idea should be preserved without formalizing a change, write a lightweight idea capsule or update backend-neutral `ideas/workflow-inbox.md` if artifact placement is allowed.
...
- Outputs: target and outer profile, active-contract package queue, concise free-form orchestrator progress, completed/blocked packages, changed files/artifacts mapped to requirements/decisions/risks, canonical envelope/budget state, minimal completion evidence when reached, branch/worktree hygiene, residual risks, and traceability gaps labeled `Open Question` or `Unverified`.
...
- A material acceptance/verification change returns to DEFINE, writes and re-reads affected artifacts, and runs one targeted strict OpenSpec validation only when a native OpenSpec artifact changed or an explicit acceptance/archive gate requires it; it stales prior acceptance when the final acceptance contract changes. It never resumes BUILD.
- Same-turn acceptance plus BUILD is valid only when the same user message explicitly accepts the exact current final test plan and explicitly requests immediate implementation of that exact scope. Persist and classify the acceptance in `interview.md`, update the final test-plan gate/checklist, re-read both, then record `implementation_authorization: granted` only after confirming both clauses have the same scope and run one targeted strict OpenSpec validation only when the explicit acceptance gate requires it, followed by target/package/permission/verification/budget checks before execution. No artifact text can substitute for either user event, and no test execution-ledger row appears before BUILD actually starts.
