...
Preserve the queue in portable package contracts, not a mandatory Markdown Board. The shared `formal-task-board.md` guide owns ordinary/formal `todo.md` current actions and `progress.txt` useful history; neither mirrors package contracts or Journal Agent/job/turn/settlement state. `tasks.md` remains accepted scope. ROSE is the sole writer of the main task's TODO/Progress; maintenance introduces no file/format gate.
