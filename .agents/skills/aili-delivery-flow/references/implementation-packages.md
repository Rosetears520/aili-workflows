...
For formal work after stable identity and package decomposition, derive the queue from the accepted contract and keep Agent/job/turn/join/settlement state in the runtime Journal. `tasks.md` remains accepted scope. The shared `formal-task-board.md` guide owns ordinary/formal `todo.md` current actions and `progress.txt` useful history; neither mirrors package contracts or runtime state. ROSE is the sole writer of the main task's TODO/Progress; maintenance introduces no file/format gate. Legacy Boards remain unparsed human notes.
...
- Map accepted task IDs to current execution packages in the orchestrator's working contract. One package may aggregate multiple accepted task IDs only when owner, dependency, join, independently completable scope, acceptance, and evidence boundaries align. Split a task during DEFINE before readiness when different canonical owners, independent joins, or independently completable scopes are required. Markdown notes do not validate or authorize this mapping.
