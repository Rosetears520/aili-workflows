...
Direct work is the default. Task use is optional and independently benefit-gated, with at most two concurrent lanes by default. Each Task context receives one bounded assignment, returns one terminal result or failure, never delegates, and is never resumed through an old `task_id`. A failed, empty, blocked, or partial result does not trigger an automatic fresh retry; later same-type use requires a new benefit decision and a fresh context with no prior `task_id`. ROSE retains lifecycle, integration, reconciliation, and final-verdict ownership. A package never creates automatic review, test, security, verifier, full-suite, or repair-loop lanes. Verification is the smallest fresh check that proves the exact claim.
