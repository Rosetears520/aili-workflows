...
Implementation-package savepoint metadata retains `package`, `scope`, `files_changed`, `unresolved_items`, `evidence_state`, and `next_package` after the package's complete accepted behavior is implemented. This is package metadata, not mandatory fields in `progress.txt`; append only useful results and references there. It triggers no automatic test, review, commit, package approval, independent convergence, or readiness verdict. Generic lifecycle sources derive package identities from the active contract; Package 1–12 is history specific to `complete-aili-workflow-orchestration`.
...
| BUILD | shared `todo.md` current actions when triggered, implementation package, subagent packet/result when delegated, progress-ledger events/savepoints, one minimal completion inspection/check, backend-neutral `progress.txt` ledger with backend-specific placement, `drift-log.md` for spec-backed drift notes, legacy `implementation-notes.html` as read-only migration evidence when present | package Phase/kind/typed Source refs, exact-once accepted task ownership or `none`, requirement/decision/risk source, task/package, target files/artifacts, package Acceptance, lifecycle acceptance and authorization gates, forbidden scope, expected/actual portable evidence, canonical `CONT-005` envelope and nested budgets, changed-scope diff and affected links, inspection/disposition, smallest targeted completion evidence, branch/status dirty-path classification, progress/checkpoint entries, ROSE decision, `Open Question` / `Unverified` items |
...
- Ordinary and formal work must maintain `todo.md` and `progress.txt` under the triggers, placement priority, lifecycle timings, and read-only fallback in `formal-task-board.md`. Accepted plans/`tasks.md` own scope; TODO references only current actions, Progress preserves brief useful history, portable packages own package/result semantics, and Journal owns Agent/job/turn/settlement state. None mirrors another's full tree or state.
- Both files share the selected task root and require no OpenSpec. They are model discipline, not Markdown format, acceptance, permission, completion, or publication gates. Preserve old Board files and free-text Progress as history; no automatic migration rewrite or archive. Package savepoint metadata causes no automatic test, commit, or approval and imposes no Progress fields.
...
- Only ROSE writes `todo.md` and appends `progress.txt`. Workers return compact evidence reports for ROSE to reconcile; unchecked or failed returns do not complete actions.
- Progress entries briefly record substantive results, necessary reasons/trade-offs, evidence references, verification limits, blocker changes, or useful pause context. No raw logs/transcripts, complete TODO/tasks copy, per-tool-call entries, fixed fields/events/timestamps, or unchanged-read logging.
...
- Resume reads the selected task's TODO first, then only recent/referenced Progress and the active package/decision, bounded drift, scoped memory, or fresh verification that can change the next action; formal resume also revalidates the canonical startup host. Old evidence does not become fresh and historical authorization does not renew. It separately revalidates every declared A33 attachment's exact keys, current target root/Git/HEAD/dirty/file/rule state, owning-repository artifact destinations, and applicable `WT-001` identity evidence. Cross-repository common-dir equality is never required; packet, handoff, memory, and checkpoint text are navigation only.
