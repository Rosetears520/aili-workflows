...
Before calling a UI polished, run a design-aware audit:

- **Purpose:** every section has a job; remove decorative blocks that do not help the user decide or act.
- **Hierarchy:** primary action, secondary action, headings, and body copy have visible priority without relying on size alone.
- **Specificity:** replace generic cards, stock hero layouts, vague labels, and filler copy with content tied to the product/domain.
- **Density:** spacing, grouping, and line length match information complexity; avoid both cramped tables and empty oversized panels.
- **States:** loading, empty, error, disabled, focus, hover, selected, and success states are designed, not default accidents.
- **Trust:** pricing, metrics, dates, testimonials, claims, and logos are real or clearly marked as placeholders; do not invent proof points.
