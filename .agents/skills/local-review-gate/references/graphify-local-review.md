...
After all preflight controls pass, the launcher passes these literal arrays with `subprocess.run([...], shell=False)` under enforceable network denial:
...
4. isolated private `HOME`, `XDG_CONFIG_HOME`, `XDG_CACHE_HOME`, `XDG_DATA_HOME`, and `TMPDIR`;
