...
| `https://github.com/affaan-m/ECC` | MIT | `commands/orch-review.md` at SHA `5216c7d...`; `commands/build-fix.md` at SHA `568351b...`; `commands/multi-plan.md` at SHA `b50912b...`; `commands/multi-execute.md` at SHA `167a9b5...`; `commands/multi-backend.md` at SHA `95cc95d...`; `commands/multi-frontend.md` at SHA `fc1c402...`; `commands/multi-workflow.md` at SHA `5458945...` | HIGH from current task evidence and focused fetch | Orchestration and repair-loop references for fail-closed fan-out, blocking/advisory split, adversarial verification, one-error-at-a-time repair, phase plan/execute/audit concepts | active-reference | Activated only in internal references and protocols; public `multi-*` commands, external workflow runtimes, and remote mutation behavior are rejected. |
...
| `https://github.com/addyosmani/agent-skills` | MIT | `skills/code-review-and-quality/SKILL.md` at SHA `5efda7a...`; `references/orchestration-patterns.md` at SHA `09cddd3...` | HIGH from current task evidence and focused fetch | Five-axis review skill details, change sizing, verification story, severity, direct invocation, single-persona command, fan-out with merge, user-driven pipelines, research isolation | active-reference | Activated as AILI review and orchestration rules; Claude-specific platform sections are not active in this OpenCode repository. |
