...
Define the repository rule for targeted Darwin optimization of the two newly added workflow skills while preventing generated artifacts under skill directories and preserving safety boundaries.

## Requirements

### Requirement: Targeted Darwin baseline for two skills
The system SHALL evaluate only `skills/explain-by-allegory/SKILL.md` and `skills/evidence-scoped-retrospective/SKILL.md` for targeted two-skill Darwin optimization unless the user explicitly expands scope.
...
- **WHEN** BUILD begins for a targeted two-skill Darwin optimization
...
The system SHALL NOT create new files or subdirectories under `skills/explain-by-allegory/` or `skills/evidence-scoped-retrospective/` during targeted Darwin optimization.
