...
- **Protocols**: versioned package and Agent-selection schemas under `core/protocols/`; lifecycle, continuity, artifact, and human-readable subagent guidance under `.agents/skills/aili-delivery-flow/references/`, with practical selection guidance under `.agents/skills/parallel-subagent-dispatch/references/`.
...
- Formal continuity: `.agents/skills/aili-delivery-flow/references/formal-task-board.md` defines optional unparsed human notes and orchestrator-owned free-form `progress.txt`; neither is a protocol or gate, and Workers do not write `progress.txt`.
...
|---|---|---|
| Active-contract implementation packages | Implement complete assigned behavior in dependency order; preserve exact file ownership and append only useful free-form status, evidence, blockers, and next action to orchestrator-owned `progress.txt` | No Markdown grammar, replay, or validation gate; tests/checkers are risk/need-triggered feedback, not automatic savepoint work, package approval, closure, or release readiness |
