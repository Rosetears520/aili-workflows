...
当前 `manifests/upstream-references.json` 已记录四组 canonical destination：`write-skills`、`requirements-grilling`、`idea-refine`、`spec-driven-development`；`write-skills` 使用本地 `SKILL.md + GLOSSARY.md`，固定上游全文仍只作 inert provenance。
