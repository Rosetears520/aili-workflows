...
| Command enters the wrong lifecycle mode | command, delivery skill | `commands/*.md`, `skills/aili-delivery-flow/references/lifecycle.md` | command route or lifecycle wording fix |
| Skill triggers too often or not enough | skill routing | target `SKILL.md` frontmatter description, `skills/using-agent-skills/SKILL.md` | description/boundary fix |
| Agent skips required lifecycle gate | delivery skill, command, agent runtime charter | `skills/aili-delivery-flow/**`, `agents/rose.md` only if always-loaded invariant is missing | lifecycle reference fix first; prompt fix only with approval |
| Subagent result lacks evidence or scope | subagent packet/result protocol | `skills/aili-delivery-flow/references/protocols/subagent-*.md`, relevant agent file | packet/result contract fix |
| Completion claim lacks proof | verification/tool policy | `skills/verification-before-completion`, `skills/review-pipeline`, `docs/harness/tool-policies.md` if in source repo | verification gate or closeout fix |
| Memory writeback/retrieval is missing or unsafe | memory | `skills/rose-memory/SKILL.md`, memory CLI evidence | memory workflow fix, no raw DB edits |
